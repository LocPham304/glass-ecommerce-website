function initAdminPromotionCreatePage() {
  ...
  const codeInput = form?.querySelector('input[placeholder="COMBO20"]');
  const titleInput = form?.querySelector('input[placeholder="Ví dụ: Combo tháng 4"]');
  ...
  const updatePreview = () => {
    ...
    if (valuePreview) {
      if (type === "cash") {
        valuePreview.textContent = `Giảm giá: ${Number(value || 0).toLocaleString("vi-VN")}d`;
      } else if (type === "ship") {
    ...
    if (maxPreview) {
      maxPreview.textContent =
        type === "percent"
          ? `Tối đa: ${Number(max || 0).toLocaleString("vi-VN")}d`
